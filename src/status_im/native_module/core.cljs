...
(defn change-database-password [key-uid account-data current-password# new-password# callback]
  (log/debug "[native-module] change-database-password")
  (init-keystore
   key-uid
   #(.changeDatabasePassword ^js (status) account-data current-password# new-password# callback)))
